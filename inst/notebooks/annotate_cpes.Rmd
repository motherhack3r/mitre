...
if (p_kind %in% c("all", "RASA")) {
  ...
  if (file.exists(file.path(p_data_path, paste0("cpes_", p_type,"_rasa.rds")))) {
    df_rasa <- readRDS(file.path(p_data_path, paste0("cpes_", p_type,"_rasa.rds")))
  } else {
    df_rasa <- mitre::nlp_cpe_annotate(type = p_type, kind = "RASA")
    saveRDS(df_rasa, file.path(p_data_path, paste0("cpes_", p_type,"_rasa.rds")))
  }
  readr::write_csv(x = df_rasa,
                   file = file.path(p_data_path,paste0("cpes_", p_type,"_rasa.csv")),
                   col_names = FALSE)
}

if (p_kind %in% c("all", "entities")) {
  # Load CPEs annotated with ENTITIES
  if (file.exists(file.path(p_data_path, paste0("cpes_", p_type,"_entities.rds")))) {
    df_enti <- readRDS(file.path(p_data_path, paste0("cpes_", p_type,"_entities.rds")))
  } else {
    df_enti <- mitre::nlp_cpe_annotate(type = p_type, kind = "entities")
    saveRDS(df_enti, file.path(p_data_path, paste0("cpes_", p_type,"_entities.rds")))
  }
  readr::write_csv(x = df_enti,
                   file = file.path(p_data_path,paste0("cpes_", p_type,"_entities.csv")),
                   col_names = FALSE)
}
```
...
if (p_kind %in% c("all", "RASA")) {
  cpes_custom <- nlp.ner_cpe_trainset(kind = "RASA", type = p_type, 
                                      ...
                                      rdataset = file.path(p_data_path, paste0("cpes_", p_type,"_rasa.rds")),
                                      seed = p_seed) %>%
...
if (p_kind %in% c("all", "entities")) {
  cpes_custom <- nlp.ner_cpe_trainset(kind = "entities", type = p_type, 
                                      num_samples = p_num_samples, 
                                      rdataset = file.path(p_data_path, paste0("cpes_", p_type,"_entities.rds")),
                                      seed = p_seed) %>%
    select(title, annotated)
  readr::write_csv(x = cpes_custom, 
                   file = file.path(name_folder, 
                                    paste0("cpes_entities_", p_type,"_", 
                                           round(p_num_samples / 1000, 1), "k.csv")), 
                   col_names = FALSE)
}
