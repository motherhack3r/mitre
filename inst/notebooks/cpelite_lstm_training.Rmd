...
```{r }
codename <- "cpe_version"

col2predict <- "version"
seed <- 42
num_samples <- 18000  # Number of samples to train on.
test_size <- 0.1 # Slice percentage from num_samples
...
```{r custom_train_Set}
df <- mitre::nlp_cpe_build_ner_trainset(num_samples = num_samples, seed = seed)
...
```{r }
spliter <- sample(c(rep(0, (1 - test_size) * nrow(df)), 
                  rep(1, test_size * nrow(df))))

df_train <- df[spliter == 0, ] 
df_test <- df[spliter == 1, ] 
```
