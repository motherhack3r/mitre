...
STRATEGY: PCA based on length and char count features
...
# Create type tag: vpv, vp, pv, vv, vend, prod or vers
...
```{r pca_summary}
acum_prop <- params$acum_prop 
pca <- prcomp(dplyr::select(df, -c("id", "title", "part", "vendor", "product", "version")), scale = T)
pca_sum <- summary(pca)
