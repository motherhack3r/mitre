...
vignette: >
  %\VignetteIndexEntry{lstm_training}
...
col2predict <- "version"
seed <- 43
num_samples <- 350000  # Number of samples to train on.
...
modelconf <- list(col2predict, seed, num_samples, test_size, 
                  validation_size, batch_size, epochs, latent_dim, activation_function)
...
```{r custom_train_Set}
raw_cpes <- readRDS("../../extdata/cpe.nist.rds")

df <- mitre::nlp_cpe_dataset(df = raw_cpes)
...
```{r}
if (globalchars) {
  cpes <- readRDS("../../extdata/train4lstm.rds")
  # cpes <- cpes[cpes$id %in% mytrain$id, ]
  cpes$title <- tolower(cpes$title)
  cpes$version <- tolower(cpes$version)
} else {
  cpes <- nlp_cpe_sample_dataset(df = df, num_samples = num_samples, seed = seed)
}
...
```{r }
spliter <- sample(c(rep(0, (1 - test_size) * nrow(cpes)), 
                  rep(1, test_size * nrow(cpes))))

df_train <- cpes[spliter == 0, ] 
df_test <- cpes[spliter == 1, ] 
```


## Vectorize the strings (input / target)

```{r }
input_texts  <- df_train$title
target_texts <- paste0('\t',df_train[, col2predict],'\n')
...
if (globalchars) {
  input_characters  <- sort(unique(c(input_characters, enc_valid_chars())))
  target_characters <- sort(unique(c(target_characters, tolower(enc_valid_chars(add_tab = T, add_enter = T)))))
...
decoder_dense   <- layer_dense(units = num_decoder_tokens, activation = activation_function)
...
```{r }
history <- model %>% fit(list(encoder_input_data, decoder_input_data), decoder_target_data,
...
```{r }
save_model_hdf5(model,paste0('lstm_', codename, "_", col2predict, '_span.h5'))
save_model_weights_hdf5(model,paste0('lstm_', codename, "_", col2predict, '_span-wt.h5'))
```
...
decoder_outputs <- decoder_dense(decoder_results[[1]])
decoder_model   <- keras_model(inputs  = c(decoder_inputs, decoder_states_inputs),
                               outputs = c(decoder_outputs, decoder_states))

```

```{r }
save_model_hdf5(encoder_model,paste0('lstm_', codename, "_", col2predict, '_inference_encoder.h5'))
save_model_weights_hdf5(encoder_model,paste0('lstm_', codename, "_", col2predict, '_inference_encoder-wt.h5'))
save_model_hdf5(decoder_model,paste0('lstm_', codename, "_", col2predict, '_inference_decoder_model.h5'))
save_model_weights_hdf5(decoder_model,paste0('lstm_', codename, "_", col2predict, '_inference_decoder_model-wt.h5'))
```
...
file.copy(from=plots.png.paths, to="models/")
