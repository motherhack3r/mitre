...
vignette: >
  %\VignetteIndexEntry{lstm_seq2seq}
...
```{r custom_train_Set}
lstm_train_version <- function(cpes = mitre::cpe.nist, num_samples = 1000, seed = 42) {
  cpes <- cpes %>% 
    select("title", "version") %>%
    sample_n(num_samples)
  cpes$title <- mitre::str73enc(cpes$title)
  cpes$version <- mitre::str49enc(cpes$version)
  
  return(cpes)
}

cpes <- lstm_train_version(cpes = mitre::cpe.nist, num_samples = num_samples, seed = seed)

```

### Load official CPE database

Select the column to be predicted by the model: vendor, product or version

```{r }
# load("/DEVEL/code/rpackages/mitre/data/cpe.nist.rda")
# cpe_colmod <- "version"
# 
# cpes$title <- mitre::cpe_encode_name(cpes$title, lower = FALSE)
# cpes[[all_of(cpe_colmod)]] <- mitre::cpe_encode_name(cpes[[all_of(cpe_colmod)]])
# 
# cpes <- cpes %>% 
#   select("title", cpe_colmod) %>%
#   sample_n(num_samples)
...
target_texts <- paste0('\t',df_train$version,'\n')
...
save_model_hdf5(model,'title2version_span.h5')
save_model_weights_hdf5(model,'title2version_span-wt.h5')
```
