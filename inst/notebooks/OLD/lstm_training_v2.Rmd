...
```{r custom_train_Set}
# raw_cpes <- mitre::standards$cpe$cpe.nist
raw_cpes <- readRDS("../extdata/cpe.nist.rds")

# remove rows with escaped chars because of tagging regex
df <- raw_cpes[!grepl(pattern = "\\\\", raw_cpes[, col2predict]), ]
# replace WFN _ to space
# df[, col2predict] <- stringr::str_replace_all(df[, col2predict], "_", " ")
# select trainable rows
df <- cpe_add_features(df)
# traincol <- paste0("train_", col2predict)
# traincol <- df[, traincol]
# df <- df[traincol, ]
