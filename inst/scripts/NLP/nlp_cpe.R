...
#' Returns latest Official CPE as data frame grouped by vendor and product.
#'
#' @param df data.frame
...
#' Prepare CPE data frame valid for LSTM training.
#'
#' @param df data.frame
#' @param verbose logical
#'
...
#' Encode CPE columns ready for LSTM input.
#' Title is encoded to 73 valid characters.
#' Vendor, product and version are encoded to 49 valid chars.
#' If any character encoding fails, the CPE is filtered.
#'
#' @param df data.frame
#'
#' @return data.frame
...
cpe_wfn2str <- function(name = character()) {
  ...
  if (length(name) > 1) {
    ...
    cat("[!] Wrong input length in function cpe_wfn2str.")
    name <- name
  }
  return(name)
}
